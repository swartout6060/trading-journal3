...
async function loadCloudData(uidValue) {
  ...
  try {
    const [tradeSnap, backtestSnap, settingsSnap, consistencySnap] = await Promise.all([
      db.collection("users").doc(uidValue).collection("trades").get(),
      db.collection("users").doc(uidValue).collection("backtestRecords").get(),
      db.collection("users").doc(uidValue).collection("settings").doc("main").get(),
      db.collection("users").doc(uidValue).collection("consistency").doc("main").get()
    ]);
    const cloudTrades = [
...
async function saveCloudData() {
  if (!cloudReady || suppressCloudSave || !currentUser || !db) return;
  try {
...
async function writeCloudSnapshot(uidValue, trades, settings) {
  const userRef = db.collection("users").doc(uidValue);
